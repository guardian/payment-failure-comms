...
case class SFCompositeResponse(responses: Seq[SFResponse]) {
  ...
  lazy val errorsAsString: Option[String] =
    if hasErrors then Some(s"Composite Request failed with: ${responses.filter(!_.success).map(_.errorAsString)}")
    else None
}

case class SFResponse(id: Option[String], success: Boolean, errors: Seq[SFResponseError]) {
  def errorAsString: Option[String] = if success then None else Some(s"Errors: ${errors.map(_.errorAsString)}.")
}
